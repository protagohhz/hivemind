...
class DHT(mp.Process):
    ...
    * trainers find most suitable experts via RemoteMixtureOfExperts (beam_search.py)
    ...
    async def _get_visible_address(
        self, num_peers: Optional[int], peers: Sequence[Endpoint], future: Optional[MPFuture]
    ):
        ...
        if peers:
            ...
            for endpoint in possible_endpoints:
                ...
                if chosen_address is not None and address != chosen_address:
                    logger.warning(
                        "At least two peers returned different visible addresses for this node:"
        ...
        if chosen_address:
            ...
            future.set_exception(
                ValueError(
                    f"Can't get address: DHT node has no peers and no public endpoint."
