...
def test_load_state_from_peers():
    ...
    averager1 = TestAverager(
        [torch.randn(3), torch.rand(5)], dht=dht1, start=True, prefix="demo-run", target_group_size=2
    )

    dht2 = hivemind.DHT(initial_peers=initial_peers, start=True)
    dht2.get("demo-run.all_averagers")
    averager2 = TestAverager(
        [torch.randn(3), torch.rand(5)], dht=dht2, start=True, prefix="demo-run", target_group_size=2
    )
