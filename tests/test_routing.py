...
def test_routing_table_search():
    for table_size, lower_active, upper_active in [(10, 10, 10), (10_000, 800, 1100)]:
        ...
        all_active_neighbors = list(chain(*(bucket.nodes_to_endpoint.keys() for bucket in routing_table.buckets)))
        ...
        for i in range(1000):
            ...
            assert all(
                our_endpoint == routing_table[our_node] for our_node, our_endpoint in zip(our_knn, our_endpoints)
            )
