...
async def test_quic_transport():
    ...
    client = await P2P.create(
        quic=True, host_maddrs=[Multiaddr(f"/ip4/127.0.0.1/udp/{client_port}/quic")], bootstrap_peers=nodes
    )
    await client.wait_for_at_least_n_peers(1)
...
async def test_call_unary_handler(should_cancel, replicate, handle_name="handle"):
    ...
    async def ping_handler(request, context):
        ...
        return dht_pb2.PingResponse(
            peer=dht_pb2.NodeInfo(node_id=server.id.to_bytes()), sender_endpoint=context.handle_name, available=True
        )
    ...
    expected_response = dht_pb2.PingResponse(
        peer=dht_pb2.NodeInfo(node_id=server.id.to_bytes()), sender_endpoint=handle_name, available=True
    )
