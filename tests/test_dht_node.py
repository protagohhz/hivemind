...
def run_protocol_listener(port: int, dhtid: DHTID, started: mp.synchronize.Event, ping: Optional[Endpoint] = None):
    ...
    logger.debug(f"Started, peer id={protocol.node_id} port={port}")

    if ping is not None:
        loop.run_until_complete(protocol.call_ping(ping))
    started.set()
    loop.run_until_complete(protocol.server.wait_for_termination())
    logger.debug(f"Finished, peer id={protocol.node_id} port={port}")
...
def test_dht_protocol():
    ...
    for listen in [False, True]:  # note: order matters, this test assumes that first run uses listen=False
        ...
        logger.debug(f"Self id={protocol.node_id}")
        ...
        (recv_value_bytes, recv_expiration), nodes_found = loop.run_until_complete(
            protocol.call_find(f"{LOCALHOST}:{peer1_port}", [key])
        )[key]
        recv_value = hivemind.MSGPackSerializer.loads(recv_value_bytes)
        (recv_id, recv_endpoint) = next(iter(nodes_found.items()))
        assert (
            recv_id == peer2_id and ":".join(recv_endpoint.split(":")[-2:]) == f"{LOCALHOST}:{peer2_port}"
        ...
        (recv_dict, recv_expiration), nodes_found = loop.run_until_complete(
            protocol.call_find(f"{LOCALHOST}:{peer1_port}", [nested_key])
        )[nested_key]
        ...
        assert LOCALHOST in loop.run_until_complete(
            protocol.get_outgoing_request_endpoint(f"{LOCALHOST}:{peer1_port}")
        )
...
def test_empty_table():
    ...
    empty_item, nodes_found = loop.run_until_complete(protocol.call_find(f"{LOCALHOST}:{peer_port}", [key]))[key]
    ...
    (recv_value_bytes, recv_expiration), nodes_found = loop.run_until_complete(
        protocol.call_find(f"{LOCALHOST}:{peer_port}", [key])
    )[key]
    recv_value = hivemind.MSGPackSerializer.loads(recv_value_bytes)
...
def test_dht_node():
    ...
    me = loop.run_until_complete(
        DHTNode.create(initial_peers=random.sample(dht.keys(), 5), parallel_rpc=10, cache_refresh_before_expiry=False)
    )
    ...
    that_guy = loop.run_until_complete(
        DHTNode.create(
            initial_peers=random.sample(dht.keys(), 3),
            parallel_rpc=10,
...
async def test_dhtnode_validate(fake_endpoint="127.0.0.721:*"):
    ...
    with pytest.raises(ValidationError):
        node2 = await DHTNode.create(
            blacklist_time=999, initial_peers=[f"{LOCALHOST}:{node1.port}"], endpoint=fake_endpoint
        )
